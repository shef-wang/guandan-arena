...
def collate_batch(batch: list[Sample]) -> tuple[Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor]:
    ...
    for index, sample in enumerate(batch):
        ...
        chosen_action_indices[index] = sample.chosen_action_index
        old_log_probs[index] = sample.old_log_prob
        target_returns[index] = sample.target_return
        advantages[index] = sample.advantage
...
def main() -> None:
    ...
    parser.add_argument("--output-dir", default="training/danzero_mvp/checkpoints/ppo_run_001")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        epoch_record = {
            "epoch": epoch,
            "loss": round(total_loss / total_examples, 6),
            "policy_loss": round(total_policy_loss / total_examples, 6),
            "value_loss": round(total_value_loss / total_examples, 6),
            "entropy": round(total_entropy / total_examples, 6),
            "mean_ratio": round(total_ratio / total_examples, 6),
            "clip_fraction": round(total_clip_fraction / total_examples, 6),
            "approx_kl": round(total_approx_kl / total_examples, 6),
