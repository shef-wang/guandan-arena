...
PPO_EPOCHS_AUTO="${PPO_EPOCHS_AUTO:-0}"
PPO_EPOCHS_MIN="${PPO_EPOCHS_MIN:-4}"
PPO_EPOCHS_MAX="${PPO_EPOCHS_MAX:-10}"
PPO_TARGET_KL="${PPO_TARGET_KL:-0.015}"
...
FULL_EVAL_EVERY="${FULL_EVAL_EVERY:-5}"
FULL_EVAL_MATCHES="${FULL_EVAL_MATCHES:-75}"
EVAL_WORKERS="${EVAL_WORKERS:-8}"
EVAL_METRIC="${EVAL_METRIC:-pair_level}"
STOP_STREAK_FULL_EVAL_ONLY="${STOP_STREAK_FULL_EVAL_ONLY:-1}"
STOP_WHEN_BEAT_FULL_EVAL_ONLY="${STOP_WHEN_BEAT_FULL_EVAL_ONLY:-1}"
...
SNAPSHOT_EVERY_ITERS="${SNAPSHOT_EVERY_ITERS:-20}"
SNAPSHOT_MAX="${SNAPSHOT_MAX:-20}"
...
for ((iter=1; iter<=ITERATIONS; iter++)); do
  ...
  if (( ROLLOUT_WORKERS <= 1 || ROLLOUT_MATCHES <= 1 )); then
    ...
    HYBRID_LEGACY_FRACTION="${HYBRID_LEGACY_FRACTION:-0.1}" \
    FROZEN_PRIOR_PROB="${FROZEN_PRIOR_PROB:-0.3}" \
...
PY
  ...
  if (( DO_EVAL == 1 )); then
    ...
    if [[ "${EVAL_METRIC:-per_game}" == "pair_level" ]]; then
      if (( MATCHES_THIS_EVAL % 2 == 1 )); then
        MATCHES_THIS_EVAL=$((MATCHES_THIS_EVAL - 1))
        echo "[scorenet] pair_level: trimmed eval matches to even count -> $MATCHES_THIS_EVAL"
      fi
      PAIRS_TOTAL=$((MATCHES_THIS_EVAL / 2))
      if (( EVAL_WORKERS > PAIRS_TOTAL )); then
        echo "[scorenet] pair_level: capping EVAL_WORKERS at pair count ($PAIRS_TOTAL) (was $EVAL_WORKERS)"
        EVAL_WORKERS="$PAIRS_TOTAL"
      fi
    ...
    echo "[scorenet] evaluating vs $OPPONENT_PROFILE... ($EVAL_LABEL, matches=$MATCHES_THIS_EVAL, metric=${EVAL_METRIC:-per_game}, workers=$EVAL_WORKERS)"
    if (( EVAL_WORKERS <= 1 || MATCHES_THIS_EVAL <= 1 )); then
      ...
      EVAL_METRIC="${EVAL_METRIC:-per_game}" \
      linebuf node "$EVAL_BUNDLE" | tee "$RUN_DIR/eval_summary.json"
    else
      EVAL_SHARD_DIR="$RUN_DIR/eval_shards"
      mkdir -p "$EVAL_SHARD_DIR"
      EVAL_PAIR_LEVEL_SHARDING=0
      if [[ "${EVAL_METRIC:-per_game}" == "pair_level" ]]; then
        EVAL_PAIR_LEVEL_SHARDING=1
        PAIRS_TOTAL=$((MATCHES_THIS_EVAL / 2))
        BASE_PAIRS_PER_WORKER=$((PAIRS_TOTAL / EVAL_WORKERS))
        REMAINDER_PAIRS=$((PAIRS_TOTAL % EVAL_WORKERS))
        echo "[scorenet] pair_level eval sharding: pairs=$PAIRS_TOTAL workers=$EVAL_WORKERS"
      else
      ...
      for ((worker=0; worker<EVAL_WORKERS; worker++)); do
        if (( EVAL_PAIR_LEVEL_SHARDING == 1 )); then
          SHARD_PAIRS="$BASE_PAIRS_PER_WORKER"
          if (( worker < REMAINDER_PAIRS )); then
            SHARD_PAIRS=$((SHARD_PAIRS + 1))
          fi
          if (( SHARD_PAIRS <= 0 )); then
            continue
          fi
          SHARD_MATCHES=$((SHARD_PAIRS * 2))
        else
          SHARD_MATCHES="$BASE_MATCHES_PER_WORKER"
          if (( worker < REMAINDER_MATCHES )); then
            SHARD_MATCHES=$((SHARD_MATCHES + 1))
...
aggregate = {
    ...
    "signedNetLevelFromLearned": 0,
    "signedNetLevelPerGame": 0,
...
for path in summary_files:
    ...
    aggregate["signedNetLevelFromLearned"] += int(d.get("signedNetLevelFromLearned", 0))
    ...
    if aggregate["evalMetric"] is None:
        aggregate["evalMetric"] = d.get("evalMetric")
    pm = d.get("pairMirror") or {}
    if pm.get("pairs"):
        if aggregate["pairMirror"] is None:
            aggregate["pairMirror"] = {"pairs": 0, "pairWins": 0, "netLevelDeltaTotalAcrossPairs": 0}
        aggregate["pairMirror"]["pairs"] += int(pm.get("pairs", 0))
        aggregate["pairMirror"]["pairWins"] += int(pm.get("pairWins", 0))
        aggregate["pairMirror"]["netLevelDeltaTotalAcrossPairs"] += int(
            pm.get("netLevelDeltaTotalAcrossPairs", 0)
        )
...
aggregate["signedNetLevelPerGame"] = aggregate["signedNetLevelFromLearned"] / matches
...
pm = aggregate.get("pairMirror")
if pm is not None and int(pm.get("pairs", 0)) > 0:
    p = int(pm["pairs"])
    pm["pairWinRate"] = float(pm["pairWins"]) / p
    pm["netLevelDeltaPerPair"] = float(pm["netLevelDeltaTotalAcrossPairs"]) / p
...
PY
    ...
    if [[ "${STOP_STREAK:-0}" -gt 0 ]]; then
      DO_STREAK=1
      if [[ "${STOP_STREAK_FULL_EVAL_ONLY:-0}" == "1" ]] && (( IS_FULL_EVAL == 0 )); then
        DO_STREAK=0
      fi
      if [[ "$DO_STREAK" -eq 1 ]]; then
...
pm = data.get("pairMirror") or {}
lm = data.get("learned") or {}
if pm.get("pairWinRate") is not None:
    win_rate = float(pm["pairWinRate"])
else:
    win_rate = float(lm.get("winRate", 0))
if "signedNetLevelFromLearned" in data:
    net = float(data["signedNetLevelFromLearned"])
else:
...
label = "pairWinRate" if pm.get("pairWinRate") is not None else "winRate"
if state["streak"] >= required:
    print(
        f"[scorenet] streak stop: {state['streak']}/{required} strong evals in a row "
        f"(signedNet={net:.3f} >= {min_net}, {label}={win_rate:.4f} >= {min_wr})"
    )
    sys.exit(0)
print(
    f"[scorenet] eval streak {state['streak']}/{required}: signedNet={net:.3f}, {label}={win_rate:.4f}"
)
sys.exit(1)
PY
        then
    ...
    if [[ "$STOP_TRAINING" -eq 0 && "$STOP_WHEN_BEAT" == "1" ]]; then
      DO_BEAT=1
      if [[ "${STOP_WHEN_BEAT_FULL_EVAL_ONLY:-0}" == "1" ]] && (( IS_FULL_EVAL == 0 )); then
        DO_BEAT=0
      fi
      if [[ "$DO_BEAT" -eq 1 ]]; then
...
if ok:
    print(
        f"[scorenet] stop condition reached: signedNetLevel={net:.3f} "
        f"(>= {min_net}), winRate={win_rate:.4f} (>= {min_wr})"
    )
    sys.exit(0)
print(
    f"[scorenet] continue training: signedNetLevel={net:.3f}, "
    f"winRate={win_rate:.4f} (target signedNet>={min_net}, winRate>={min_wr})"
)
sys.exit(1)
PY
        then
