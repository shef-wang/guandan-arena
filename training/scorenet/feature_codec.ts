import type { ArenaActionOption, ArenaCardView, ArenaPlayView, ArenaTurnInput } from '../../src/arena/types';
import {
  buildGreedyPlanStats,
  createPlanningCache,
  rankLegacyV1ActionCandidates,
  type PlanStats,
  type PlanningCache,
} from '../../src/game/ai';
import { sameTeam } from '../../src/game/rules';
import type { Card, GameState, PlayType, Rank, Seat, Suit } from '../../src/game/types';
import codecConstants from './codec_constants.json';

export const STATE_DIM = codecConstants.STATE_DIM;
export const ACTION_DIM = codecConstants.ACTION_DIM;
export const MAX_ACTIONS = codecConstants.MAX_ACTIONS;

const RANKS: Rank[] = codecConstants.RANKS as Rank[];
const SUITS: Suit[] = codecConstants.SUITS as Suit[];
const PLAY_TYPES: PlayType[] = codecConstants.PLAY_TYPES as PlayType[];

const MAX_NORMAL_RANK_COUNT = 8;
const MAX_ACTION_RANK_COUNT = 4;

export interface EncodedTurn {
  stateFeatures: number[];
  actionFeatures: number[][];
}

export interface HeuristicContext {
  cache: PlanningCache;
  actionScoreMap: Map<string, number>;
  orderedActionKeys: string[];
  scoreMean: number;
  scoreStd: number;
  passScore: number;
}

export function buildHeuristicContext(state: GameState, seat: Seat): HeuristicContext {
  const cache = createPlanningCache();
  const candidates = rankLegacyV1ActionCandidates(state, seat);
  const actionScoreMap = new Map<string, number>();

  for (const candidate of candidates) {
    if (candidate.type === 'pass') {
      actionScoreMap.set('pass', candidate.score);
      continue;
    }
    actionScoreMap.set(`play:${candidate.play!.key}`, candidate.score);
  }

  const orderedCandidates = [...candidates].sort((a, b) => b.score - a.score);
  const orderedActionKeys = orderedCandidates.map((candidate) =>
    candidate.type === 'pass' ? 'pass' : `play:${candidate.play!.key}`,
  );

  const scores = candidates.map((candidate) => candidate.score);
  const scoreMean = scores.length > 0 ? scores.reduce((sum, value) => sum + value, 0) / scores.length : 0;
  const variance =
    scores.length > 1 ? scores.reduce((sum, value) => sum + (value - scoreMean) ** 2, 0) / scores.length : 1;
  const scoreStd = Math.max(1e-6, Math.sqrt(variance));
  const passScore = actionScoreMap.get('pass') ?? scoreMean;

  return {
    cache,
    actionScoreMap,
    orderedActionKeys,
    scoreMean,
    scoreStd,
    passScore,
  };
}

export function encodeTurnForPolicy(input: ArenaTurnInput, heuristic: HeuristicContext): EncodedTurn {
  return {
    stateFeatures: encodeStateFeatures(input),
    actionFeatures: input.legalActions.map((action) => encodeActionFeatures(action, input, heuristic)),
  };
}

function encodeStateFeatures(input: ArenaTurnInput): number[] {
  const ownRankCounts = countRanks(input.hand, MAX_NORMAL_RANK_COUNT);
  const ownSuitCounts = countSuits(input.hand);
  const seenRankCounts = countRanks(input.publicKnowledge.seenCards, MAX_NORMAL_RANK_COUNT);
  const finishedFlags = input.players.map((player) => (player.finished ? 1 : 0));
  const finishPositions = input.players.map((player) => (player.finishPosition ?? 0) / 4);
  const roundPassFlags = ([0, 1, 2, 3] as const).map((seat) => (input.roundTrace[seat].action === '不出' ? 1 : 0));
  const handCounts = input.players.map((player) => player.handCount / 27);

  const tableOwnerIsTeammate =
    input.currentTablePlay && sameTeam(input.seat, input.currentTablePlay.owner) ? 1 : 0;
  const consecutivePasses = input.roundTrace[input.seat].action === '不出' ? 1 : 0;
  const legalActionRatio = input.legalActions.length / MAX_ACTIONS;

  return [
    ...ownRankCounts,
    ...ownSuitCounts,
    countWild(input.hand) / 4,
    input.hand.length / 27,
    ...handCounts,
    ...oneHot(input.currentPlayer, 4),
    ...oneHot(input.seat, 4),
    ...encodeOwner(input.currentTablePlay?.owner ?? null),
    ...encodePlayType(input.currentTablePlay?.play ?? null),
    (input.currentTablePlay?.play.size ?? 0) / 8,
    (input.currentTablePlay?.play.primaryValue ?? 0) / 16,
    (input.currentTablePlay?.play.bombSize ?? 0) / 8,
    input.currentTablePlay?.play.usesWild ? 1 : 0,
    (input.currentTablePlay?.play.wildCount ?? 0) / 4,
    ...finishedFlags,
    ...finishPositions,
    ...roundPassFlags,
    ...seenRankCounts,
    tableOwnerIsTeammate,
    consecutivePasses,
    legalActionRatio,
  ];
}

function encodeActionFeatures(action: ArenaActionOption, input: ArenaTurnInput, heuristic: HeuristicContext): number[] {
  const play = action.play;
  const cards = play?.cards ?? [];
  const rankCounts = countRanks(cards, MAX_ACTION_RANK_COUNT);
  const suitCounts = countSuits(cards);
  const remainingCards = getRemainingCards(action, input.hand);
  const remainingRankCounts = countRanks(remainingCards, MAX_NORMAL_RANK_COUNT);
  const remainingSuitCounts = countSuits(remainingCards);

  const actionKey = action.kind === 'pass' ? 'pass' : action.actionId;
  const rawScore = heuristic.actionScoreMap.get(actionKey) ?? heuristic.scoreMean;
  const normalizedScore = clamp((rawScore - heuristic.scoreMean) / heuristic.scoreStd, -5, 5) / 5;
  const normalizedPassScore = clamp((heuristic.passScore - heuristic.scoreMean) / heuristic.scoreStd, -5, 5) / 5;
  const rankIndex = heuristic.orderedActionKeys.indexOf(actionKey);
  const rankRatio =
    heuristic.orderedActionKeys.length > 1
      ? (rankIndex >= 0 ? rankIndex : heuristic.orderedActionKeys.length - 1) / (heuristic.orderedActionKeys.length - 1)
      : 0;

  const remainingStats = buildRemainingPlanStats(remainingCards, heuristic.cache);
  const bombLikeCount = remainingStats.bombs + remainingStats.straightFlushes + remainingStats.jokerBombs;

  return [
    action.kind === 'pass' ? 1 : 0,
    ...encodeActionType(action),
    (play?.size ?? 0) / 8,
    (play?.primaryValue ?? 0) / 16,
    (play?.bombSize ?? 0) / 8,
    (play?.wildCount ?? 0) / 4,
    play?.usesWild ? 1 : 0,
    ...rankCounts,
    ...suitCounts,
    ...remainingRankCounts,
    ...remainingSuitCounts,
    countWild(remainingCards) / 4,
    remainingCards.length / 27,
    normalizedScore,
    normalizedPassScore,
    rankRatio,
    Math.min(remainingStats.turns, 16) / 16,
    Math.min(bombLikeCount, 6) / 6,
    Math.min(remainingStats.singles, 12) / 12,
  ];
}

function buildRemainingPlanStats(cards: ArenaCardView[], cache: PlanningCache): PlanStats {
  const internalCards = cards.map(toInternalCard);
  return buildGreedyPlanStats(internalCards, cache);
}

function toInternalCard(card: ArenaCardView): Card {
  return {
    id: card.id,
    suit: card.suit,
    rank: card.rank,
    deck: card.deck,
    isWild: card.isWild,
  };
}

function encodeActionType(action: ArenaActionOption): number[] {
  const values = new Array(PLAY_TYPES.length + 1).fill(0);
  if (action.kind === 'pass') {
    values[0] = 1;
    return values;
  }

  const index = PLAY_TYPES.indexOf(action.play!.type);
  values[index + 1] = 1;
  return values;
}

function encodePlayType(play: ArenaPlayView | null): number[] {
  const values = new Array(PLAY_TYPES.length + 1).fill(0);
  if (!play) {
    values[0] = 1;
    return values;
  }

  const index = PLAY_TYPES.indexOf(play.type);
  values[index + 1] = 1;
  return values;
}

function encodeOwner(owner: Seat | null): number[] {
  const values = new Array(5).fill(0);
  if (owner === null) {
    values[0] = 1;
    return values;
  }

  values[owner + 1] = 1;
  return values;
}

function countRanks(cards: ArenaCardView[], maxCount: number): number[] {
  return RANKS.map((rank) => cards.filter((card) => card.rank === rank).length / maxCount);
}

function countSuits(cards: ArenaCardView[]): number[] {
  return SUITS.map((suit) => cards.filter((card) => card.suit === suit).length / 8);
}

function countWild(cards: ArenaCardView[]): number {
  return cards.filter((card) => card.isWild).length;
}

function oneHot(index: number, size: number): number[] {
  return new Array(size).fill(0).map((_, current) => (current === index ? 1 : 0));
}

function getRemainingCards(action: ArenaActionOption, hand: ArenaCardView[]): ArenaCardView[] {
  if (action.kind === 'pass') {
    return hand;
  }

  const playedIds = new Set(action.cardIds);
  return hand.filter((card) => !playedIds.has(card.id));
}

function clamp(value: number, min: number, max: number): number {
  return Math.max(min, Math.min(max, value));
}
